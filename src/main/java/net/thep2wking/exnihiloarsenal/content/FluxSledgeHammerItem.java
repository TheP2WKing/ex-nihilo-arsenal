...
import net.thep2wking.exnihiloarsenal.api.SledgeHammerItemLikeCoFH;
import net.thep2wking.exnihiloarsenal.config.ExNihiloArsenalConfig;
import novamachina.exnihilosequentia.api.ExNihiloRegistries;

public class FluxSledgeHammerItem extends SledgeHammerItemLikeCoFH implements IMultiModeFluxItem 
{
    ...
    public FluxSledgeHammerItem(IItemTier tier, int attackDamageIn, float attackSpeedIn, int energy, int xfer, Properties builder) 
    {
        super(tier, attackDamageIn, attackSpeedIn, new HashSet<>(), builder.stacksTo(1));
    ...
    public float getDestroySpeed(@Nonnull ItemStack stack, @Nonnull BlockState state) 
    {
    	...
    	{
	        if (ExNihiloRegistries.HAMMER_REGISTRY.isHammerable(state.getBlock()) && state.getBlock().getHarvestLevel(state) <= this.getTier().getLevel()) 
	        {
	            return (float) (this.speed * ExNihiloArsenalConfig.flux_sledge_hammer_destroy_speed_modifier.get());
	        }
    	}
        return 1F;
    }
    ...
    //added attack knockback because its a hammer
    @Override
    public Multimap<Attribute, AttributeModifier> getAttributeModifiers(EquipmentSlotType slot, ItemStack stack) 
    {
        ...
        if (slot == EquipmentSlotType.MAINHAND) 
        {
            ...
            multimap.put(Attributes.ATTACK_KNOCKBACK, new AttributeModifier(UUID_TOOL_KNOCKBACK, "Tool modifier", 1, AttributeModifier.Operation.ADDITION));
